#> Core
scoreboard objectives add kgc.lg minecraft.custom:minecraft.leave_game
scoreboard objectives add sneak minecraft.custom:minecraft.sneak_time
scoreboard objectives add sneak_ dummy
scoreboard objectives add item_counter dummy

#> Settings
scoreboard objectives add meth_settings dummy

#> Methamphetamine
scoreboard objectives add meth_needed dummy
scoreboard objectives add want_use_meth dummy
scoreboard objectives add meth_used dummy
scoreboard objectives add addiction_stage dummy
scoreboard objectives add meth_addiction_time dummy
scoreboard objectives add use_meth trigger
scoreboard objectives add meth_action_time dummy
scoreboard objectives add meth_action_time_in_s dummy

#> Meth effects
scoreboard objectives add meth_speed dummy
scoreboard objectives add meth_haste dummy
scoreboard objectives add meth_night_vision dummy

#> Machines
scoreboard objectives add machine_fuel dummy
scoreboard objectives add machine_working_time dummy

#> Evaporator
scoreboard objectives add evaporator_prog_s1 dummy
scoreboard objectives add evaporator_prog_s2 dummy
scoreboard objectives add evaporator_prog_s3 dummy

#> Const
scoreboard objectives add CONST dummy
scoreboard players set tick CONST 20

#> Food and drinks 
scoreboard objectives add foodndrink trigger
scoreboard objectives add foodndrinktimer dummy